...
if (true) {
  ...
  $objPHPExcel->getActiveSheet()->setCellValue('C1', "CAPACITACION");
  ...
  foreach ($data as $capacitaciones) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('B' . $fila, $capacitaciones['nombre'].' '.$capacitaciones['apellidos']);
    $objPHPExcel->getActiveSheet()->setCellValue('C' . $fila, $capacitaciones['cantidad']=='0'?"NO":"SI");

    $fila++;
  }

  $objPHPExcel->getActiveSheet()->setSharedStyle($estiloTituloColumnas, "A1:c1");
  ...
  header('Content-Disposition: attachment;filename="Participacionencapacitaciones.xls"');
