...
if (!empty($_GET['edad_ini']) && !empty($_GET['edad_fin']) ) {
  ...
  foreach ($data as $funcionario) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('I' . $fila, $funcionario['fecha_nacimiento']);
    $objPHPExcel->getActiveSheet()->setCellValue('J' . $fila, $funcionario['edad']);
...
/*  $objPHPExcel->getActiveSheet()->setCellValue('K' . $fila, $funcionario['fecha_ingreso_nombra']);
  ...
  header('Content-Disposition: attachment;filename="Reporte por edad.xls"');
