...
if (true) {
  ...
  $data = generalesMD::ParticipacionesenCapacitaciones($_GET['fecha_ini'], $_GET['fecha_fin']);
  // if (!empty($_GET['mes']))
  ...
  class PDF extends FPDF_dd
  {
    ...
    function tableTotal($data)
    {
      ...
      $this->Cell(180, 5, utf8_encode('PARTICIPACION DE FUNCIONARIOS EN CAPACITACIONES ENTRE LAS FECHAS : '. $_GET['fecha_ini'] .' Y '. $_GET['fecha_fin']), 1, 0, 'C', true);
      $this->Ln();
      $this->SetFont('Arial', 'b', 8);
      $this->Cell(20, 5, utf8_decode('#'), 1, 0, 'C', true);
      $this->Cell(50, 5, utf8_decode('Documento'), 1, 0, 'C', true);
      $this->Cell(80, 5, utf8_decode('Nombre'), 1, 0, 'C', true);
      $this->Cell(30, 5, utf8_decode('Participacion'), 1, 0, 'C', true);
      ...
      foreach ($data as $key => $row) {
        ...
        $this->Cell(20, 5, utf8_decode($key+1), 1, 0, 'C', true);
        $this->Cell(50, 5, utf8_decode($row['documento']), 1, 0, 'C', true);
        $this->Cell(80, 5, utf8_decode($row['nombre'].' '.$row['apellidos']), 1, 0, 'C', true);
        $this->Cell(30, 5, utf8_decode($row['cantidad']==0?"NO":"SI"), 1, 0, 'C', true);
      

        /*$this->Cell(30, 5, utf8_decode($row['direccion']), 1, 0, 'C', true);
          $this->Cell(30, 5, utf8_decode($row['entidadCapacitadora']), 1, 0, 'C', true);
        $this->Cell(30, 5, utf8_decode($row['asistentes']), 1, 0, 'C', true);
         $this->Cell(20, 5, utf8_decode($row['estado']), 1, 0, 'C', true);*/
        $this->Ln(5);
      }
  ...
  $pdf = new PDF('L','mm',array(200, 200));
  $pdf->AliasNbPages();
  ...
  $name_file = 'participacionenCapacitaciones' . time() . '.pdf';
