...
if (true) {
  ...
  class PDF extends FPDF_dd
  {
    ...
    function tableTotal($data)
    {
      ...
      $this->Cell(1170, 5, utf8_encode('Detalles Funcionarios'), 1, 0, 'C', true);
      $this->Ln();
      ...
      $this->Cell(45, 5, utf8_decode('FECHA DE INGRESO'), 1, 0, 'C', true);
      ...
      foreach ($data as $key => $funcionario) {
        ...
        if ($funcionario['cabeza_familia'] == 0)
          $cabeza_familia = 'NO';
        $this->Cell(45, 5, utf8_decode($funcionario['documento']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['nombre']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['apellidos']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['tipo_vinculacion']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['nivel']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['cargo']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['codigo']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['grado']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['dependencia']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['sede']), 1, 0, 'C', true);
        $this->Cell(45, 5, utf8_decode($funcionario['fecha_ingreso_nombra']), 1, 0, 'C', true);
  ...
  $pdf = new PDF('P', 'mm', array(1200, 1200));
  $pdf->AliasNbPages();
  ...
  // $pdf->tableTotales();
  $pdf->SetFont('Arial', '', 8);
  $name_file = 'funcionarios.pdf';
  if ($_GET['num'] == 2)
    $name_file = 'funcionarios_nivel_educativo.pdf';
  if ($_GET['num'] == 1)
    $name_file = 'Funcionarios_genero.pdf';
  if ($_GET['num'] == 3)
    $name_file = 'Funcionarios_sede.pdf';
  if ($_GET['num'] == 4)
    $name_file = 'Funcionarios_dependencia.pdf';
