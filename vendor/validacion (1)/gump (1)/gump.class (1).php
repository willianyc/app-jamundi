...
class GUMP
{
     ...
     */
    public function get_errors_array($convert_to_string = null, $return_keys = true)
    {
        if (empty($this->errors)) {
            return ($convert_to_string) ? null : array();
        }
    ...
    protected function validate_alpha_numeric_space($field, $input, $param = null)
    {
        ...
        // _dd if (!preg_match("/^([a-z0-9ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖßÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿ\s])+$/i", $input[$field]) !== false) {
        if (!preg_match("/^([a-z0-9ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖßÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿñ\s])+$/i", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
