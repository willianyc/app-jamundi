$(document).ready(function () {
  ...
  $(document).on("click", "button[name=mdlAceptar]", function () {
    ...
    $("input[name=selected]:checked").each(function () {
      ...
      var sede = $(this).parents("tr").find("input[name=sede]").val();
      var codigo = $(this).parents("tr").find("input[name=codigo]").val();
...
function guardarCapacitacion() {
  ...
  data = {
    ...
    horas_formativas: $("#horas_formativas").val(),
    conocimiento_previo: $("#conocimiento_previo").val(),
    fecha_asignacion: $("#fecha_asignacion").val(),
    entidadCapacitadora: $("#entidadCapacitadora").val(),
    calificacion: $("input[name=calificacion]:checked").val(),
    funcionariosAsignados: funcionariosAsignados,
  };
  $.post("set/capacitacion/crear", {
...
function getFuncionarioAll() {
  $.post("ajaxGeneral.php?mode=funcionariosAll").done(function (data) {
    if (data.trim() !== "") {
      ...
      $(data).each(function () {
        tr += `<tr>
            ...
            <td scope="col" name="tipo_vinculacion">${this.tipo_vinculacion}</td>
            <td scope="col" name="nivel">${this.nivel}</td>
            <td scope="col" name="cargo">${this.cargo}</td>
            </tr>`;
      });
