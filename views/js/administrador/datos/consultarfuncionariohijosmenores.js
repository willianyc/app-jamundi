      ...
      if (data.trim() !== '') {
        ...
        $(data).each(function () {
         ...
         <td name="nombre"><a href="https://app-jamundi.fksas.com/index.php?view=hojavida&mode=crear&id_funcionario=${this.id_funcionario}">
          ...
          <td name="nombre_hijo">${this.nombre_hijo===null?"":this.nombre_hijo}</td>
          <td name="fecha_nac">${this.nacimiento_hijo===null?"":this.nacimiento_hijo}</td>
          <td name="edad_hijo">${this.edad_hijo===null?"":this.edad_hijo}</td>
          </tr>`;
          i++;
        });
...
function loadReporteEcxel() {
  ...
  if (fecha_ini != '' && fecha_fin != null) {
    newwindow = window.open("?view=reportes&mode=reportesFuncionarioMenoresEdadEcxel&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, "width=815,height=460")
    if (newwindow == null) {
...
function loadReportePDF() {
  ...
  if ( fecha_ini != '' && fecha_fin != null) {
    ...
    newwindow = window.open("?view=reportes&mode=reportefuncionariohijosmenorespdf&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin , 'popUp', opciones)   
