...
if (!fecha_fin) {
  ...
  $.post('ajaxGeneral.php?mode=getFuncionarioIngreso', {
      fecha_fin: fecha_fin,
      fecha_ini: fecha_ini
    })
    .done(function (data) {
      if (data.trim() !== '') {
        ...
        $(data).each(function () {
          ...
          <td name="fecha_nac">${this.fecha_ingreso_nombra===null?"":this.fecha_ingreso_nombra}</td>
          </tr>`
          i++
        });
...
function loadReporteEcxel() {
  ...
  if (fecha_ini != '' && fecha_fin != null) {
    newwindow = window.open("?view=reportes&mode=reportefuncionarioIngresoexcel&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, "width=815,height=460")
    if (newwindow == null) {
...
function loadReportePDF() {
  ...
  if (fecha_ini != '' && fecha_fin != null) {
    ...
    newwindow = window.open("?view=reportes&mode=reportefuncionarioIngresopdf&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, 'popUp', opciones)
		if (newwindow == null) {
		  alert('Configure el navegador para que permita las ventanas emergentes en esta pagina');
		} else {
		  newwindow.creator = self
