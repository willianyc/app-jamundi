$(document).ready(function () {
 $("#btnBuscar").on("click", function () {
    ...
    $('#MsjAJX').html('')
    ParticipacionesenCapacitaciones()
    return
    });
...
function ParticipacionesenCapacitaciones() {
  var fecha_i = $("#fecha_ini").val();
  var fecha_f = $("#fecha_fin").val();
 $.post('ajaxGeneral.php?mode=ParticipacionesenCapacitaciones', {
      fecha_fin: fecha_f,
      fecha_ini: fecha_i
    }).done(function (data) {
    if (data.trim() !== "") {
...
function loadReporteEcxel() {
  ...
  newwindow = window.open("?view=reportes&mode=participacionCapacitacionesexcel&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, "width=815,height=460")
  if (newwindow == null) {
...
function loadReportePDF() {
 var fecha_ini = $("#fecha_ini").val();
  ...
  if (fecha_ini != '' && fecha_fin != null) {
    ...
    newwindow = window.open("?view=reportes&mode=reporteparticipacionencapacitacionpdf&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, 'popUp', opciones)
    if (newwindow == null) {
      alert('Configure el navegador para que permita las ventanas emergentes en esta pagina');
    } else {
      newwindow.creator = self
