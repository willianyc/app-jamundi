$(document).ready(function () {
 $("#btnBuscar").on("click", function () {
    ...
    $('#MsjAJX').html('')
    rankingFuncionariosCapacitacion()
    return
    });
...
function rankingFuncionariosCapacitacion() {
  var fecha_i = $("#fecha_ini").val();
  var fecha_f = $("#fecha_fin").val();
 $.post('ajaxGeneral.php?mode=rankingFuncionariosCapacitacion', {
      fecha_f: fecha_f,
      fecha_i: fecha_i
    }).done(function (data) {
    if (data.trim() !== "") {
      ...
      $(data).each(function () {
        ...
        <td name="rankingFuncionarios">${this.capacitaciones===null?"":this.capacitaciones}</td>
        <td name="participaciones">${this.participaciones===null?"":this.participaciones}</td>
        </tr>`;
      });
...
function loadReporteEcxel() {
  ...
  newwindow = window.open("?view=reportes&mode=rankingfunCapacitacionesexcel&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, "width=815,height=460")
  if (newwindow == null) {
...
function loadReportePDF() {
 var fecha_ini = $("#fecha_ini").val();
  ...
  if (fecha_ini != '' && fecha_fin != null) {
    ...
    newwindow = window.open("?view=reportes&mode=rankingCapacitacionespdf&fecha_ini=" + fecha_ini + "&fecha_fin=" + fecha_fin, 'popUp', opciones)
    if (newwindow == null) {
      alert('Configure el navegador para que permita las ventanas emergentes en esta pagina');
    } else {
      newwindow.creator = self
