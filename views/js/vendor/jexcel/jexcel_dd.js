/**
 * (c) jExcel v3.3.3_dd.1
 * 
 * Se modifico un error que no dejaba cargar el archivo
 * 
...
var jexcel = (function (el, options) {
        ...
        for (var i = 0; i < size; i++) {
            ...
            if (obj.options.columns[i].type == 'autocomplete' || obj.options.columns[i].type == 'dropdown') {
                ...
                if (obj.options.columns[i].url) {
                    ...
                    requests.push(fetch(obj.options.columns[i].url, {
                            headers: new Headers({
    ...
    obj.setValue = function (cell, value, force) {
        ...
        let _dd_deleteJqueryKeysInCell = function(cell){
            Object.keys(cell).forEach(function(key){
                key.indexOf("jQuery") == 0 && delete cell[key];
            })
            return cell
        }
        ...
        if (typeof (cell) == 'string') {
            ...
            cell = _dd_deleteJqueryKeysInCell(cell)
            var keys = Object.keys(cell);
            if (obj.isPropJquery(keys)) {
                keys.pop()
            }
            if (keys.length > 0) {
                for (var i = 0; i < keys.length; i++) {
    ...
    // _dd
    obj.isPropJquery = function (keys) {
        let isJqProp = false;
        if (keys.length == 1) {
            let index = keys[0].match(/^jQuery\d+$/)
            if (index != undefined) {
                isJqProp = true
            }
        }
        ...
        workbook.SheetNames.forEach(function (sheetName) {
            ...
            if (temp && temp.length) {
                for (var i = 0; i < temp.length; i++) {
                    ...
                    // if(temp[i] ==undefined) throw new Error('Error wpx')
                    try {
